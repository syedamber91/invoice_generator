...
def generate_pdf(items_df, letterhead_bytes, header_info):
    ...
    for i, row in enumerate(items_df.itertuples(index=False), start=1):
        ...
        # greedily — page 1 fills up to the letterhead's bottom QR band
        # before we break to a new page. Totals overflow to page 2 alone
        # if they don't fit below the last item (handled separately below
        # the loop).
